...
- **One terminal is now one agent — the worst first-run papercut in the product (found by the QUICKSTART gate).** The CLI minted a fresh advisory identity (`<host>-<pid>-<rand4>`) per invocation, so two consecutive commands from the same shell looked like two competing agents: `daimon start web` took a 30s soft lock and the `daimon stop web` typed a second later was **denied by it** — exit 5, "locked by agent …", on the second command a new user ever runs. The identity is now derived from the parent process (the shell), so a terminal keeps one identity across invocations while a second terminal, an editor, or a Claude Code session each still get their own and the multi-agent protection they exist for. Shape unchanged; identity remains advisory; an explicit `DAIMON_AGENT_ID` still wins.
...
- **Suite: 1130 tests, 0 fail** (v1.13: 1102), dashboard vitest 140 (v1.13: 130). New test files: `init-wizard`, `quickstart`, `tui-first-run`, `doctor-onboarding`.
