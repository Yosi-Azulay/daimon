...
A pre-tag review pass over this release found these, all fixed here:

- **Any query containing `word:` used to 400** on the `stable` `GET /api/search` — `TypeError: cannot read`, `http://localhost:4200`, `node:fs` all worked in v1.15 and stopped working. The grammar now errors only on a near-miss of a real field name (Levenshtein ≤ 2, the same threshold daimon has used for verb and config-key suggestions since M91).
- **Backslashes were consumed as escapes anywhere**, so `D:\src\app.ts` searched for `D:srcapp.ts` — on daimon's own primary platform — and quoting it (the remedy the error suggested) made it worse. A backslash now escapes only a quote, a space, or another backslash.
- **`level:warning` could never match a log line.** The grammar's levels are error/warning/lint; the v1.2 log column stores error/warn/info/debug. The mapping is now explicit (`warning` → `warn`), and `level:lint` matches lint events only — documented, not silently empty.
- **An out-of-range time crashed the TUI.** `after:1751328000000000000` (a pasted nanosecond timestamp) reached `new Date(…).toISOString()` on the search pane's render path and threw inside the daemon process. Times outside the Date range are no longer times.
- **A long query could 500 the endpoint.** ~1000 AND'd LIKE clauses exceed SQLite's expression-depth limit, from a code path outside the try/catch. The LIKE path now takes the same 8-token cap the FTS path always had (which also fixes the fallback returning *fewer* rows than the index for a 9+ term query), and the column path is wrapped.
- **`""` returned the newest 50 rows as "hits"** — a query with no terms and no filters is now refused with a remedy.
- **`?scope=all&app=api` scoped every store except error groups**, which came back from all apps and all time.
- Smaller: `?limit=-1` silently dropped a hit (now clamped); `POST /api/searches/<anything>` was treated as "save" (exact paths only); a saved search named `.` or `..` could be created but never deleted (refused); the TUI's saved-search list rendered queries without stripping control characters, so a saved ESC sequence could repaint the pane.
### Notes
...
- **`daimon graph [--json] [--workspace <label>] [--all]` + `GET /api/graph` + MCP `daimon_graph` (M175, all experimental).** The READ-ONLY view of the graph daimon already computes: nodes with live status/health/effective-workspace/group-membership, edges from `config.depends` restricted to known apps, topo levels (the exact order orchestrate would use), `cycles[]` naming every dependency cycle, and `unordered[]` for apps blocked downstream of one. A readable indented tree on a TTY; the endpoint body when piped or with `--json`. It renders — it never starts, stops, or reorders anything, and `test/graph.test.mjs` greps the module for process-touching APIs to keep it that way. MCP is now **35 tools**.
- **Dashboard graph page (M174)** — `/graph`, a new lazy chunk in the Observe nav group (`g y`). Hand-rolled SVG, no chart/graph/layout library: columns are the daemon's topo levels (dependencies left — what you see is literally the start order), node color comes from the status tokens with a distinct glyph AND border treatment per status (never color-only), cycle members get a flagged banner naming the cycle path. **Keyboard + aria from day one**: every node is tabbable in topo order, arrows walk the graph itself (left → dependency, right → dependent, up/down → column), Enter opens the app detail, and an offscreen paragraph narrates the whole graph — counts, start order, cycles, blocked apps — for screen readers. Nodes recolor on the existing refresh path; no reload.
- **Group clusters + the `up` start-order preview (M176).** v1.1 groups render as labeled background hulls on the graph page (an app in several groups is drawn once, inside each hull), and the group panel previews `up <group>`'s plan — "will start — level 1: api · level 2: web" — computed by the **same** `groupUpPlan` call the verb executes, carried on `/api/graph`, so preview and behavior cannot drift. `daimon up <group> --dry-run` (new, additive) prints that plan and starts nothing; on a TTY the same line renders as chrome before a real run. **Zero behavior change**: ordering, timeouts, exit codes, and the piped JSON shapes of `up` are byte-identical to v1.14.
- **Workspace switcher, dashboard (M173).** The v1.12 header pill is now a real switcher: it lists every configured searchRoot — an unlabeled root shows as its folder basename, so a label-free config still gets a usable switcher — plus "All workspaces". The preference persists in **localStorage per browser**; reload restores it; an explicit `?workspace=` deep-link wins over both the stored choice and the `?cwd=` auto-pick. Never a daemon-global: two browsers can watch two different workspaces at once.
- **Workspace filter chord, TUI (M173).** `w` cycles none → each workspace → none (effective labels, config order); the active filter shows in the v1.13 status bar as `ws:<label>` with the `visible/total` count. The filter lives in the TUI process's own state — never written to the daemon or `state.json` (grep-asserted), so two attached TUIs can hold two different filters simultaneously.
- **`?workspace=` end-to-end (M177).** `/api/errors`, `/api/search`, and `/api/history/trends` gain the param (experimental, additive — absent param is byte-identical); `daimon errors --workspace <label>` (flat all-apps shape, no name needed) and `daimon search --workspace` expose it; `daimon report --workspace` already existed. A parameterized parity suite (`test/workspace-parity.test.mjs`) asserts filtered = unfiltered ∩ members, row-for-row, per surface.

### Changed

- **One workspace-matching rule everywhere (M177).** Every `?workspace=` surface now matches by the EFFECTIVE label — the searchRoot's label, or its folder basename when unlabeled (`discovery.ts`'s own labelHint fallback) — via one shared helper. Before, an unlabeled root's apps were unfilterable: `?workspace=<basename>` returned `[]` on every surface. Strictly additive: labeled matching is unchanged; previously-empty results become useful ones.
- **Unknown workspace labels now error with the known ones named — on every surface whose param is new (M177, the M90 rule).** `?workspace=nope` used to silently return `[]` — a typo'd label looked like an empty workspace. The surfaces gaining the param in v1.15 (`/api/errors`, `/api/search`, `/api/history/trends`, `/api/graph`) and the experimental `/api/report` now send the same 400: `unknown workspace: nope`, `known: [...]`, and a remedy; the CLI relays it with exit 1. The frozen `/api/apps` and stable `/api/overview` accepted the param long before v1.15, so their historical 200-with-empty response is **unchanged** — additive-only is the law on those tiers (a review pass caught the 400 leaking onto them before release).
- **Dashboard pages scope to the switcher (M177).** Home, tests, timeline, trends, and report now follow the workspace bus like apps and errors always did — no page silently shows all-workspace data while the header claims a filter. Sessions, agents, settings, and doctor stay deliberately daemon-global.

### Notes

- **The graph is read-only visualization, permanently.** No cascade behavior changes, no auto-restart logic, no dependency-driven side effects; `depends.ts` / `orchestrate.ts` are consumed, never modified. Graph editing stays out of scope — depends is hand-written config.
- **Hand-rolled SVG only** — zero new dependencies; the graph page is a lazy chunk and the initial-payload gzip budget (<150KB) holds.
- **New surfaces all ship `experimental`**: the `graph` verb, `GET /api/graph`, MCP `daimon_graph`, `up --dry-run`, the `?workspace=` params on errors/search/trends, and both switchers.

## [1.14.0] — 2026-07-21

"First Run" (M168–M172) — the release that lowers the doorstep. Every doc daimon shipped was written for its author; a stranger's first fifteen minutes decide adoption, and those fifteen minutes had a wizard that hard-coded four marker files while the daemon knew twenty-seven frameworks, a dashboard that met an empty workspace with an empty table, a README that opened with architecture, and silence where "I ran it in the wrong directory" should have produced a finding. Nothing here records anything new, and nothing phones home. **Migration: none — every v1.13 surface keeps its shape.**

### Added

- **QUICKSTART.md — executable documentation (M170).** The five-minute path: install, the three-line hand-written config *first* (the wizard is optional, and documented as such), `daimon init` as the shortcut, daemon start, first list/start/stop, and `daimon doctor` when it breaks. `test/quickstart.test.mjs` executes **every** `bash` block on the page, in order, against a clean `DAIMON_HOME` — only the install and browser-opening blocks are exempt, each with a stated reason the test asserts. The page cannot rot without failing the suite.
- **`daimon init --yes` (M168).** Non-interactive: accepts the scanned proposal exactly and prints the proposal JSON it accepted (shape pinned in `test/fixtures/contract/cli-init-proposal.json`, tier `experimental`). Refuses outright when a config already exists — `--force` remains the documented override.
- **Four first-run doctor rules (M171)**, all suggest-only, all naming a remedy: `config-wrong-directory` (config lives in a parent, or only the global fallback was found — and it never invents a `--config` flag, because there isn't one), `daemon-not-started` (nothing at all listening on apiPort, distinct from the port-holder rules that own the "something else has it" case), `no-apps-detected` (surfaces the single likeliest cause from discovery's own `stats.rejected` tally rather than a generic guess), and `port-pool-absent` (detected profiles declare `portFlag`/`portEnv` but no `ports.pool` is set). Each appears in the generated doctor coverage table.
- **Dashboard first-run states (M169).** Zero configured apps now renders a walkthrough card on `/` and `/apps` — the copy-able `daimon init` command, what happens next, and a pointer to QUICKSTART — instead of an empty table; the errors/tests/report/logs empty states explain what will appear there and which action produces it. Dismissal is **localStorage only**; no telemetry, no phone-home, no request that is not loopback.
- **A TUI first-attach hint (M170).** One line, on the first attach this machine has ever had, pointing at v1.13's `?` overlay. The key it names is read from the chord map, so a remap can never leave the hint lying. Acknowledged in `state.json` (`tuiHintSeen`, additive, merge-written) on mount — so a crash between attach and quit cannot make it reappear.

### Changed

- **`daimon init` is now a UI over the real discovery scan (M168).** Its private four-marker list (`nx.json` / `angular.json` / `vite.config.*` / `.storybook`) is **deleted**, not extended: init runs `discoverApps()` with cwd as the proposed searchRoot, so it proposes exactly what `daimon list` will find, and every framework in the registry — Django, Rails, .NET, Flutter, the `package.json` fallback — lights up in the wizard for free. A new profile row now reaches init with no init change at all.
- **init writes exactly one file, and starts nothing.** `daimon.config.json` in cwd, asserted by a filesystem-sentinel test. The legacy `~/.daimon/config.json` wizard target and the Claude-install side quest are gone from the flow (that file is still honored if it exists — only the prompt is gone); the closing lines *point at* `daimon daemon start` and `daimon claude install` rather than running them.
- **README leads with the first run.** Install and the first-run path are the first screen, with QUICKSTART linked from it; architecture and reference moved below the fold.
- Config-missing hints across the CLI and API now say `daimon init --yes` rather than the older `--auto`.

### Fixed

- **One terminal is now one agent — the worst first-run papercut in the product (found by the QUICKSTART gate).** The CLI minted a fresh advisory identity (`<host>-<pid>-<rand4>`) per invocation, so two consecutive commands from the same shell looked like two competing agents: `daimon start web` took a 30s soft lock and the `daimon stop web` typed a second later was **denied by it** — exit 5, "locked by agent …", on the second command a new user ever runs. Identity is now keyed to the **terminal session** (`WT_SESSION` / `TERM_SESSION_ID` / `ITERM_SESSION_ID` / `TMUX_PANE` / `SSH_TTY`, falling back to the parent pid, then to per-process), with the minted random suffix remembered per session in `~/.daimon/cli-sessions.json` under a 12h TTL. A second terminal, an editor, or a Claude Code session each still get their own identity and the multi-agent protection they exist for. Shape unchanged (`<host>-<n>-<4hex>`); identity remains advisory; an explicit `DAIMON_AGENT_ID` still wins.
  - Two properties the first attempt at this got wrong, both caught in review and fixed before release: `process.ppid` is **not** stable in Git Bash/MSYS (it forks per command, so a ppid-keyed id silently reverted to per-process there), and deriving the 4-hex suffix from the session key left it with **zero entropy** — on OS pid reuse two different shells produced a byte-identical id, and `LockManager.acquire` then *refreshed* the other agent's live lock instead of denying it, with no denial, no steal, and no audit row. The suffix is real randomness again.
- **`daimon init` could be tricked into writing outside your project.** `existsSync` follows symlinks, so a dangling `daimon.config.json -> …` read as "no config here": `--yes` skipped its refusal, the interactive flow showed the friendly *create* prompt, and the write then landed on the link's target anywhere on disk. Occupancy is now decided with `lstat`, writing *through* a symlink is refused outright, and the config write is tmp+rename so a crash can't truncate a hand-written file.
- **`daimon daemon start` in the docs.** The five-minute path documented a foreground command as if it backgrounded — the executable-docs gate caught it on its first run. It reads `daimon daemon start --detach`, and says what the flag does.
- **Legacy `?tab=` deep links landed on the home page (shipped since v1.12).** `scrollToSection` rewrote the fragment with `history.replaceState(state, '', '#' + id)` — and a **bare** fragment resolves against `<base href="/">`, throwing the `/apps/<name>` path away. So `/apps/web/?tab=errors` (a documented permanent back-compat shape since M85) landed on `/#errors`, the overview home, and any section scroll corrupted the address bar so a copied URL pointed at the wrong page. The fragment is now written with the path and query preserved. Deep-link back-compat is a hard rule in this repo, and this was a real break.
- **The deep-link gate no longer skips itself.** `e2e/redirects.spec.ts` called `test.skip()` for every `needsApp` route when no app was registered on the daemon under test — so in an e2e environment without apps it reported green while verifying nothing, which is exactly how the `?tab=` break shipped through two releases. It now fails loudly with a remedy instead. Verified at 31/31 against a daemon with a mixed multi-framework workspace.

### Notes

- **Zero new HTTP endpoints, config keys, dependencies, or history migrations.** The dashboard work recomposes what `daimon-api.ts` already exposed; nothing landed in `server.ts`. `tuiHintSeen` in state.json is the only new persisted field, additive and optional. No frozen shape moved. Every new surface is `experimental`; `init` keeps its `stable` tier and its `--auto`/`--force` semantics.
- **No telemetry, still and always.** The tour asks nothing and reports nothing; first-run state is local (localStorage for the dashboard, `~/.daimon/state.json` for the TUI).
- **Suite: 1137 tests, 0 fail** (v1.13: 1102), dashboard vitest 140 (v1.13: 130). New test files: `init-wizard`, `quickstart`, `tui-first-run`, `doctor-onboarding`.
- **`daimon doctor` keeps its v1.13 exit semantics.** The two new first-run rules initially pushed `ok:false`, which made `doctor` exit 1 whenever the daemon simply wasn't running and broke the `daimon doctor && daimon daemon start` idiom. Both are `ok:true` advisories now, matching every other suggest-only rule.
- **The quickstart gate runs alone.** It starts a real daemon and a real dev server, and inside the 40-way parallel suite that load made the timing-sensitive files (`plugin-isolation`, `resource-sampling`) flake 1–2 tests a run. `npm test` now runs the parallel suite first and this one integration gate after it, sequentially. **No perf budget was loosened to achieve that** — the fix was removing the contention, not accommodating it; `resource-sampling.test.mjs` keeps its committed thresholds byte-for-byte, and the suite went green three runs in a row.
