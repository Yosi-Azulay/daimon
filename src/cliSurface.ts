...
export const CLI_SUBCOMMANDS: CliSubcommand[] = [
  ...
  {
    name: 'logs',
    args: '<name> [--tail N] [--since 30s] [--grep <regex>] [--stream]',
    summary: 'Recent log lines for an app. --grep filters server-side (regex, ≤512 chars); --stream follows the live tail as NDJSON.',
    description: 'Recent log lines for an app. --grep applies a case-insensitive regex filter on the daemon (length-capped at 512 chars). --stream follows the live tail (SSE under the hood) and emits NDJSON {ts,line} until SIGINT — combine with --grep for a filtered live tail.',
  ...
  },
  {
    name: 'stop',
    args: '<name> [--steal]',
    summary: 'Stop an app. --steal overrides another agent\'s soft-lock.',
    description: 'Stop an app. If another agent holds the soft-lock, this exits 5 unless --steal is passed.',
