...
export const CLI_SUBCOMMANDS: CliSubcommand[] = [
  ...
  {
    name: 'errors',
    args: '<name> [--since 2m] [--since-last] [--client <id>] [--structured] [--full|--compact] | --group [<group>] | --workspace <label>',
    summary: 'Get deduplicated errors for an app. Compact by default ({file,line,col,code,message}). --group (no name) groups all apps\' errors by stack fingerprint; --workspace (no name) scopes to one workspace.',
    description: 'Deduplicated parsed errors for an app. Compact JSON by default. With --group and no app name, returns all apps\' errors grouped by stack fingerprint (source location or normalized message) with count, first/last-seen and affected apps. With --workspace <label> and no app name (v1.15, experimental), returns every app\'s errors in that workspace, flat shape; unknown labels error naming the known ones.',
    ...
    options: [
      ...
      { flag: '--workspace', arg: '<label>', description: 'All errors from one workspace, flat shape (omit <name>; v1.15, experimental).' },
  ...
  {
    name: 'search',
    args: '<query> [--all] [--app <a>] [--workspace <label>] [--since <dur>] [--kind logs|errors|events|tests|error-groups] [--limit N]',
    ...
    description: 'FTS5 search across the history DB — per-app log lines (indexed by default; opt out with search.logIndex or overrides.<app>.logIndex), plus all errors and events. A trailing * on a term does prefix search. Degrades to a LIKE scan (fallback:true) if FTS is unavailable.\n\nThe query accepts the v1.16 syntax: `app:<name>`, `kind:<facet>`, `level:error|warning|lint`, `before:`/`after:` (2026-07-01, 2026-07-01T14:30, 24h, epoch ms), "quoted phrases", and bare terms — all ANDed. Filters compile to WHERE clauses on real columns, so they behave identically on the FTS index and on the LIKE fallback. An unknown field is an error naming the valid ones; quote a token to search for it literally. --all (or kind:tests / kind:error-groups) widens the scope to test runs and folded error groups and adds a per-kind `facets` count.',
    example: 'daimon search \'app:api level:error "ECONNREFUSED" after:24h\'',
    options: [
      { flag: '--all', description: 'Unified scope: also search test runs and error groups; adds facets (v1.16, experimental).' },
      { flag: '--app', arg: '<name>', description: 'Restrict to one app (app: in the query wins).' },
      { flag: '--workspace', arg: '<label>', description: 'Restrict hits to one workspace\'s apps (v1.15, experimental).' },
      { flag: '--since', arg: '<duration>', description: 'Time window (e.g. 30m, 24h, 7d) — after: in the query wins.' },
      { flag: '--kind', arg: '<k>', description: 'logs|errors|events, plus tests|error-groups with --all (default: all).' },
  ...
  {
    name: 'audit',
    args: '[--agent <id>] [--app <name>] [--since 2h] [--limit 100] [--json]',
    summary: 'Query the audit trail: who did what, when. Derives { ts, agent, action, app, changedKeys, remote } rows from audit.log + audit.log.1. Filters compose (AND); newest first.',
    description: 'Query daimon\'s append-only audit trail (audit.log + the rotated audit.log.1). Rows are derived from the existing verb:<app> convention: lifecycle actions (start/stop/restart/steal/handoff/mute/unmute/test), group actions, and config writes. --agent/--app/--since/--limit compose as AND filters; default limit 100, newest first. Malformed or truncated lines are skipped and counted (never fabricated, never an error). NOTE: agent ids are ADVISORY — a self-declared X-Daimon-Agent header, never authenticated; the trail records what each declared identity did, not who they really were.',
    example: 'daimon audit --agent host-1234-ab12 --since 2h',
    examples: ['daimon audit', 'daimon audit --app web-admin --since 1d', 'daimon audit --agent host-1234-ab12 --json'],
    options: [
      { flag: '--agent', arg: '<id>', description: 'Filter to a declared agent id (advisory, unverified).' },
      { flag: '--app', arg: '<name>', description: 'Filter to actions on one app.' },
      { flag: '--since', arg: '<duration>', description: 'Time window (e.g. 15m, 2h, 7d).' },
      { flag: '--limit', arg: '<n>', description: 'Max rows (default 100, cap 1000).' },
      { flag: '--json', description: 'Emit compact JSON.' },
    ],
    needsDaemon: true,
    group: 'agent',
    stability: 'experimental',
  ...
  },
  {
    name: 'agents',
    args: '[--json]',
    summary: 'Roster of agents touching this daemon: id, last-seen, action counts, held soft-locks, contention. Derived at query time from the audit log + live registry + lock manager.',
    description: 'Print the agent roster: each declared agent\'s id, last-seen, active flag, cwd, call count, per-action counts, and any per-app soft-locks it currently holds, plus a contention summary (waits/steals). The roster MERGES the live in-memory registry (5-min activity window) with audit-derived history — an agent that acted earlier but is idle now still appears (active: false). Rows with no declared agent aggregate under (unknown). Nothing is persisted for this — it is computed on demand. NOTE: agent ids are ADVISORY — self-declared headers, never authenticated.',
    example: 'daimon agents',
    examples: ['daimon agents', 'daimon agents --json'],
    options: [
      { flag: '--json', description: 'Emit compact JSON.' },
    ],
    needsDaemon: true,
  ...
  {
    name: 'up',
    args: '[<group>|<profile>] [--until serving|healthy] [--timeout 300s] [--steal] [--dry-run]',
    summary: 'Start a group or profile (or autoStart). Groups resolve first: depends-aware topo order + a "3/4 healthy" readiness summary. Profiles behave as before. --dry-run previews a group\'s start order without acting.',
    description: 'Start a named group or legacy profile (no argument = the autoStart list). Groups (v1.1) resolve first: the daemon starts members and their depends closure in topological order, waits per level, and returns a per-app readiness summary with a "3/4 healthy" tail (exit 2 when not all reached). Legacy profile invocations behave exactly as before. --until/--timeout/--steal apply to the group path. --dry-run (v1.15, groups only) prints the planned start order — the same topo levels `up` executes — and starts nothing; on a TTY the same preview line renders before a real run.',
    example: 'daimon up day',
    examples: ['daimon up day', 'daimon up day --dry-run', 'daimon up fullstack', 'daimon up'],
    options: [
      ...
      { flag: '--dry-run', description: 'Preview the group start order (topo levels) without starting anything (v1.15, experimental; groups only).' },
    ],
    needsDaemon: true,
    group: 'lifecycle',
    stability: 'stable',
  ...
  {
    name: 'export',
    args: '[--since 7d] [--app <a>] [--format json|md|csv] [--out <file>]',
    summary: 'One-way carry-out bundle: events, error groups, test runs, compiles, crashes (bounded tails), and the report in one versioned envelope (schemaVersion 1). JSON canonical; --format md paste-ready; --format csv flat rows.',
    description: 'Export a self-contained bundle of what happened (default window 7d): persisted events, fingerprint-folded error groups, test runs, compiles, crash reports (each keeping only its existing bounded log tail), and the M83 report — composed from existing history, never re-derived. The JSON envelope is versioned (schemaVersion 1, additive-only; readers must ignore unknown keys). Export is ONE-WAY: there is no `daimon import` and no plan for one. Redaction holds — env key names + hashes only, never values. --format md renders the report plus per-section summaries; --format csv emits flat `section,ts,app,summary,detail` rows. --out writes atomically (tmp + rename); without it the bundle goes to stdout, pipe-friendly.',
    example: 'daimon export --since 7d --out week.json',
    examples: ['daimon export --since 7d --out week.json', 'daimon export --format md', 'daimon export --app web-admin --format csv --out web.csv'],
    options: [
      { flag: '--since', arg: '<window>', description: 'Export window: 24h, 7d, or a duration (default 7d).' },
      { flag: '--app', arg: '<name>', description: 'Limit to one app.' },
      { flag: '--format', arg: '<fmt>', description: 'json (canonical, default) | md (paste-ready) | csv (flat rows).' },
      { flag: '--out', arg: '<file>', description: 'Write to a file atomically (tmp + rename) instead of stdout.' },
    ],
    needsDaemon: true,
    group: 'introspection',
    stability: 'experimental',
  ...
  {
    name: 'graph',
    args: '[--json] [--workspace <label>] [--all]',
    summary: 'READ-ONLY view of the depends graph: nodes (status/health/workspace/groups), edges from config.depends, topo levels, and any cycles. A terminal tree on a TTY; the /api/graph JSON when piped or with --json.',
    description: 'Render the dependency graph daimon already computes (v1.15 "Atlas"). Nodes are apps with their live status, health, effective workspace label (label, or the searchRoot\'s basename when unlabeled), and v1.1 group membership; edges come from config.depends restricted to known apps; topo levels show the start order orchestrate would use; cycles[] names any dependency cycle (those apps cannot be ordered). Pure visualization — it never starts, stops, or reorders anything. Scoped to the current workspace by cwd like other read verbs; --all shows every workspace; --workspace pins a label. JSON (the /api/graph body) when piped or with --json.',
    example: 'daimon graph',
    examples: ['daimon graph', 'daimon graph --json', 'daimon graph --workspace fullstack', 'daimon graph --all'],
    options: [
      { flag: '--json', description: 'Emit the /api/graph JSON even on a TTY.' },
      { flag: '--workspace', arg: '<label>', description: 'Filter to one workspace label (unknown labels error, naming the known ones).' },
      { flag: '--all', description: 'Ignore cwd scoping — the graph across every workspace.' },
    ],
    needsDaemon: true,
    group: 'introspection',
    stability: 'experimental',
  ...
  },
  {
    ...
    summary: 'Manage plug-ins (Plugin API v1): list, show one, validate a file offline.',
    description: 'Manage plug-ins. `list` shows installed plug-ins with status (same data as `daimon plugins`); `show <name>` prints one plug-in\'s row; `validate <path>` sanity-checks a plug-in file offline against the Plugin API v1 shape ({ name, apiVersion: 1, ...hooks }) without a daemon. See PLUGINS.md.',
    example: 'daimon plugin validate ./my-plugin.mjs',
    needsDaemon: true,
    group: 'plugin',
    stability: 'stable',
    exitCodes: stdExit,
  },
  {
    name: 'plugins',
    args: '[--json]',
    summary: 'List loaded plug-ins: name, apiVersion, status (active|disabled|load-error), hooks, error.',
    description: 'What loaded, what didn\'t, and why — without reading daemon logs. One row per file in ~/.daimon/plugins: name, file, apiVersion, status (`active` — loaded and hooked; `disabled` — a hook threw this session, unhooked until the next daemon restart; `load-error` — the file failed to import or validate), declared hooks, and the error message for non-active states. Plug-ins are NOT sandboxed — they run in-process with full Node privileges; see PLUGINS.md for the trust model and the v1 hook surface.',
    example: 'daimon plugins',
    options: [
      { flag: '--json', description: 'Emit compact JSON even on a TTY.' },
    ],
    needsDaemon: true,
