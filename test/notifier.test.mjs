...
test('Notifier: init log line + attempt + ok recorded for status->error', async () => {
  const tmpHome = path.join(os.tmpdir(), `daimon-notify-test-${Date.now()}`);
  ...
  const logFile = path.join(os.homedir(), '.daimon', 'notifications.log');
...
test('Notifier: throttled within 60s', async () => {
  const tmpHome = path.join(os.tmpdir(), `daimon-notify-throttle-${Date.now()}`);
  fs.mkdirSync(tmpHome, { recursive: true });
  process.env.USERPROFILE = tmpHome;
  process.env.HOMEDRIVE = tmpHome[0] + ':';
  process.env.HOMEPATH = tmpHome.slice(2);
  ...
  const log = fs.readFileSync(path.join(os.homedir(), '.daimon', 'notifications.log'), 'utf8');
